...
const skills = {
			...
			backup(result, player) {
				return {
					...
					async content(event, trigger, player) {
						...
						const list = game.createCardGroup(cards, "color");
						const result = await player
							.chooseButtonTarget({
	...
	mbcmqingzheng: {
		...
		async cost(event, trigger, player) {
			const list = game.createCardGroup(player.getCards("h"), "suit", player);
		...
		async content(event, trigger, player) {
			...
			const list = game.createCardGroup(target.getCards("h"), "suit", target);
