const translates = {
	...
	sp_puyuan: "SP蒲元",
	yj_sp_biancai: "辨材",
	yj_sp_biancai_info: "每个角色的回合开始时，你可以进行判定，红色，你从牌堆中获得一张装备牌；黑色，你从弃牌堆中获得一张装备牌。",
	yj_sp_cuiren: "淬刃",
	yj_sp_cuiren_info: "出牌阶段限一次，你可以弃置一张装备牌，并根据弃置牌的类别获得以下效果直到回合结束；武器：你使用牌无距离和次数限制；防具：你使用牌无法被响应；坐骑：你使用牌指定目标时，可以多指定任意个目标。",
	yj_sp_shenfeng: "神锋",
	yj_sp_shenfeng_info: "当你使用【杀】指定目标后，你可以弃置一张装备牌，然后选择一项：1、令此【杀】伤害+1；2、令此【杀】造成的伤害视为失去体力；3、令目标角色弃置两张牌。",
	yj_sp_huanling: "唤灵",
	yj_sp_huanling_info: "出牌阶段开始时或当你受到伤害后，你可以重铸一张牌，然后获得场上、弃牌堆或牌堆一张花色相同的装备牌，并从随机出现该装备的三个兵主技能选择其中一个令一名角色获得。若该角色不为你，你摸X张牌（X为其技能数）。",
	yj_sp_shenduan: "神锻",
