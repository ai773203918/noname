    ...
    skill: {
      ...
      init(player, skill) {
        ...
        next.setContent(async (event, trigger, player2) => {
          ...
          player2[num > 0 ? "gainMaxHp" : "loseMaxHp"](Math.abs(num));
        });
      },
  ...
  zf_zhudao: {
    ...
    skill: {
      ...
      async cost(event, trigger, player) {
        ...
        event.result = await player.chooseCard(get.prompt2(event.skill), select, (card, player2) => player2.canRecast(card)).set("ai", (card) => 6 - get.value(card)).forResult();
      },
      async content(event, trigger, player) {
        const { cards } = event;
