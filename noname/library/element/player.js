...
class Player extends HTMLDivElement {
  ...
  getName() {
    ...
    if (lib.translate[name2 + "_ab"]) {
      return lib.translate[name2 + "_ab"];
    }
    return get.translation(name2);
  }
  /**
      ...
      for (let i = 0; i < skill.contentFuns.length; i++) {
        ...
        if (typeof fun2 === "function") {
          ...
          if (exports.isSandboxRequired()) {
            recompiledScope = scope ? exports.eval(`return (${scope.toString()})`) : (code) => exports.eval(`return (${code.toString()})`);
          } else {
  ...
  *iterableGetGainableCards(player2, arg1, arg2) {
    ...
    if (broadcast !== false) {
      game.broadcast(
        function(player2, cards2, gaintag2) {
          player2.$addToExpansion(cards2, null, gaintag2);
        },
        this,
        cards,
        gaintag
  ...
  removeVirtualJudge(VCard) {
    ...
    if (VCard.storage.equipEnable && VCard.cards?.some((card) => get.type(card) == "equip")) {
      ...
      if (equips.length) {
        let keepSkills = Object.values(player2.additionalSkills).flat(), skills = get.skillsFromEquips(equips).removeArray(keepSkills);
        if (skills.length) {
          player2.removeSkill(skills);
        }
      }
...
hasSkillTag：`, arg);
  ...
  addVirtualJudge(card, cards) {
    ...
    if (cardx.storage.equipEnable && (cardx.cards || cards).some((card2) => get.type(card2) == "equip")) {
      ...
      if (equips.length) {
        let skills = get.skillsFromEquips(equips).filter((skill) => !player2.hasSkill(skill, "e"));
        if (skills.length) {
