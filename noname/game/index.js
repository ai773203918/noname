const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["_virtual/typescript.js","_virtual/_commonjsHelpers.js","node_modules/.pnpm/typescript@5.9.2/node_modules/typescript/lib/typescript.js","_virtual/_commonjs-dynamic-modules.js","_virtual/typescript2.js","_virtual/___vite-browser-external.js","_virtual/__vite-browser-external.js"])))=>i.map(i=>d[i]);
...
class Game extends GameCompatible {
   ...
   * 基于钩子的添加势力方法
   */
  addGroup(id, short, name2, config) {
    if (!id) {
      throw new TypeError();
    }
  ...
  connect(ip, callback) {
    ...
    game.sandbox = exports.createSandbox(ip);
...
${JSON.stringify(error, null, "	")}`);
  ...
  loadModeAsync(name2, callback, onerror = (e) => console.error(e)) {
    let promise = (async () => {
      ...
      let isESM = true;
      try {
        exports2 = await __vitePreload(() => import(
          /* @vite-ignore */
          `../../mode/${name2}/index.js`
        ), true ? [] : void 0);
      } catch (e1) {
      ...
      if (isESM) {
        if (!["object", "function"].includes(typeof exports2.default)) {
          throw new Error(`导入的模式[${name2}]格式不正确！`);
        }
        game.import("mode", exports2.default);
      }
  ...
  switchMode(name2, configx) {
    ...
    game.loadModeAsync(name2, async (exports2) => {
      let mode = exports2;
      _status.sourcemode = lib.config.mode;
      lib.config.mode = name2;
      for (let i in exports2.element) {
        if (!lib.element[i]) {
          lib.element[i] = [];
        }
        for (let j in exports2.element[i]) {
          if (j == "init") {
            if (!lib.element[i].inits) {
              lib.element[i].inits = [];
            }
            lib.element[i].inits.push(exports2.element[i][j]);
          } else {
            lib.element[i][j] = exports2.element[i][j];
          }
        }
      }
      for (let i in exports2.ai) {
        if (typeof exports2.ai[i] == "object") {
          if (ai[i] == void 0) {
            ai[i] = {};
          }
          for (let j in exports2.ai[i]) {
            ai[i][j] = exports2.ai[i][j];
          }
        } else {
          ai[i] = exports2.ai[i];
        }
      }
      for (let i in exports2.ui) {
        if (typeof exports2.ui[i] == "object") {
          if (ui[i] == void 0) {
            ui[i] = {};
          }
          for (let j in exports2.ui[i]) {
            ui[i][j] = exports2.ui[i][j];
          }
        } else {
          ui[i] = exports2.ui[i];
        }
      }
      for (let i in exports2.game) {
        game[i] = exports2.game[i];
      }
      for (let i in exports2.get) {
        get[i] = exports2.get[i];
      }
      if (game.onwash) {
      ...
      for (let i in exports2) {
        if (["element", "game", "ai", "ui", "get", "config", "start", "startBefore"].includes(i)) {
          continue;
        }
        if (lib[i] == void 0) {
          lib[i] = Array.isArray(exports2[i]) ? [] : {};
        }
        for (let j in exports2[i]) {
          lib[i][j] = exports2[i][j];
        }
      }
      ...
      if (exports2.startBefore) {
        exports2.startBefore();
      }
      game.createEvent("game", false).setContent(exports2.start);
      if (lib.mode[lib.config.mode] && lib.mode[lib.config.mode].fromextension) {
        let startstr = exports2.start.toString();
  ...
  addPlayerOL(target, character, character2, isNext) {
    ...
    const addPlayer = function(id2, target2, character3, character22, isNext2) {
      ...
      ui.arena.setNumber(players2.length + 1);
