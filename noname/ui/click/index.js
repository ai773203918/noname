  ...
  charactercard(name, sourcenode, noedit, resume, avatar, audioName) {
    ...
    const refreshIntro = function() {
      ...
      if (lib.config.show_characternamepinyin == "showPinyin2" || lib.config.show_skillnamepinyin == "showPinyin2" || lib.config.show_characternamepinyin == "showCodeIdentifier2" || lib.config.show_skillnamepinyin == "showCodeIdentifier2") {
        ...
        let dieAudios2 = get.Audio.die({ player: { name, skin: { name: skinName }, tempname: [skinName] } }).audioList.map((i2) => i2.text).filter(Boolean);
        const skillAudioMap = /* @__PURE__ */ new Map();
        nameinfo.skills.forEach((skill) => {
          let voiceMap = get.Audio.skill({ skill, player: { name, skin: { name: skinName }, tempname: [skinName] } }).textList;
          if (voiceMap.length) {
            skillAudioMap.set(skill, voiceMap);
          }
        });
        const derivationSkillAudioMap = /* @__PURE__ */ new Map();
        nameinfo.skills.forEach((skill) => {
          var info = get.info(skill);
          if (info.derivation) {
            ...
            for (var i2 = 0; i2 < derivation.length; i2++) {
              ...
              let derivationVoiceMap = get.Audio.skill({ skill: derivation[i2], player: { name, skin: { name: skinName }, tempname: [skinName] } }).textList;
              if (derivationVoiceMap.length) {
                derivationSkillAudioMap.set(derivation[i2], derivationVoiceMap);
              }
            }
          }
        });
        if (dieAudios2.length || skillAudioMap.size > 0 || derivationSkillAudioMap.size > 0) {
          ...
          if (skillAudioMap.size > 0) {
            ...
            skillAudioMap.forEach((texts, skill) => {
              const skillNameSpan1 = document.createElement("span"), skillNameSpanStyle1 = skillNameSpan1.style;
          ...
          if (derivationSkillAudioMap.size > 0) {
            ...
            derivationSkillAudioMap.forEach((texts, skill) => {
              const derivationSkillNameSpan1 = document.createElement("span"), derivationSkillNameSpanStyle1 = derivationSkillNameSpan1.style;
        ...
        if (dieAudios2.length || skillAudioMap.size > 0 || derivationSkillAudioMap.size > 0) {
          ...
          if (skillAudioMap.size > 0) {
            ...
            intro.appendChild(skillNameSpan);
            skillAudioMap.forEach((texts, skill) => {
              const skillNameSpan2 = document.createElement("span"), skillNameSpanStyle = skillNameSpan2.style;
              skillNameSpanStyle.fontWeight = "bold";
              skillNameSpan2.innerHTML = `<br>${get.translation(skill)}<br>`;
        ...
        clickSkill = function(e) {
          ...
          if (this.link != "dieAudios") {
            ...
            if (lib.config.background_speak && e !== "init") {
              if (!this.playAudio || name != this.audioName) {
                let audioList = get.Audio.die({ player: { name: this.playername, skin: { name } } }).fileList;
                this.playAudio = game.tryAudio({
                  audioList,
                  addVideo: false,
                  random: false,
    ...
    if (lib.characterSubstitute[name]) {
      ...
      let skin = ui.create.div(".changeskin2", "查看其他皮肤", playerbg, function() {
        ...
        skin._created = true;
        var createButtons = function(list3, skinList2) {
          if (!list3) {
            return;
          }
          if (list3.length >= 4) {
          ...
          for (let i2 of list3) {
            let button = ui.create.div(avatars2, function() {
              ...
              for (let i3 = 0; i3 < skillButtons.length; i3++) {
                delete skillButtons[i3].playAudio;
              }
              refreshIntro();
              game.callHook("refreshSkin", [list3[0], this.name]);
            });
            let iSTemp2 = false;
            if (!lib.character[i2] && skinList2.some((skin2) => skin2[0] == i2)) {
              iSTemp2 = true;
              lib.character[i2] = get.convertedCharacter(["", "", 0, [], (skinList2.find((skin2) => skin2[0] == i2) || [i2, []])[1]]);
            }
            button.name = i2;
            const skinImg = lib.character[i2]?.img;
            skinImg ? button.setBackgroundImage(skinImg) : button.setBackground(i2, "character");
            if (iSTemp2) {
              delete lib.character[i2];
            }
          }
        };
        let list2 = this.list, skinList = this.skinList;
        createButtons(list2, skinList);
      });
      skin.skinList = lib.characterSubstitute[name];
      skin.list = [name, ...lib.characterSubstitute[name].map((skin2) => skin2[0])];
    }
